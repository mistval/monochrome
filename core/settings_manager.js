  ...
  load(settingsCategoriesData, settingsCategoriesFilePaths, config) {
    ...
    for (let settingsFilePath of settingsCategoriesFilePaths) {
      try {
        ...
        categoriesFromFilesData = categoriesFromFilesData.concat(categoryDatas);
      } catch (err) {
        this.logger_.logFailure(loggerTitle, 'Failed to load settings categories from file: ' + settingsFilePath, err);
      }
    }

    try {
      let childCategories = settingsCategoriesData.concat(categoriesFromFilesData);
      this.rootSettingsCategory_ = SettingsCategory.createRootCategory(CATEGORY_IDENTIFIER, SETTING_IDENTIFIER, childCategories, config);
    } catch (err) {
