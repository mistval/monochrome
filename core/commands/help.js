...
class Help {
  constructor(commands, config) {
    ...
    this.commandsForTopLevelHelp_ = this.nonHiddenCommands_
  ...
  showAdvancedHelp_(msg, targetAlias, settings) {
    let command = findCommandWithAlias(this.nonHiddenCommands_, targetAlias);
    if (!command) {
      command = findCloseMatchCommandForAlias(this.nonHiddenCommands_, targetAlias);
    }
    if (!command) {
      return this.showGeneralHelp_(msg, settings);
    }
